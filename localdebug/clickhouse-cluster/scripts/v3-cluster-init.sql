...
AS SELECT
   ...
   WHERE (toDate(time) >= (today() - 3)) AND (toDate(time) <= (today() + 1)) AND index_type = 0
;
...
;
CREATE TABLE IF NOT EXISTS statshouse_v3_1m_history_buffer ON CLUSTER statlogs2 ENGINE = Buffer(default, statshouse_v3_1m, 1, 30, 300, 10_000, 1_000_000, 10_000_000, 100_000_000)
;
CREATE TABLE IF NOT EXISTS statshouse_v3_1h_history_buffer ON CLUSTER statlogs2 ENGINE = Buffer(default, statshouse_v3_1h, 1, 30, 300, 10_000, 1_000_000, 10_000_000, 100_000_000)
;

CREATE MATERIALIZED VIEW IF NOT EXISTS statshouse_v3_1s_matview_history ON CLUSTER statlogs2 TO statshouse_v3_1s_history_buffer
(
    `index_type` UInt8,
    `metric` Int32,
    `pre_tag` UInt32,
...
AS SELECT
    ...
    uniq_state
FROM statshouse_v3_incoming
WHERE (toDate(time) >= (today() - 3)) AND (toDate(time) <= (today() + 1)) AND index_type = 1
;



CREATE MATERIALIZED VIEW IF NOT EXISTS statshouse_v3_1m_matview_history ON CLUSTER statlogs2 TO statshouse_v3_1m_history_buffer
        (
         `index_type` UInt8,
         `metric` Int32,
         `pre_tag` UInt32,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS statshouse_v3_1h_matview_history ON CLUSTER statlogs2 TO statshouse_v3_1h_history_buffer
        (
         `index_type` UInt8,
         `metric` Int32,
         `pre_tag` UInt32,
