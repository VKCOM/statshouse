...
func generateStats(simID int, journal *metajournal.MetricsStorage, s *agent.Agent, totalRange int, key1range int32, key2range int32, key3range int32) {
	...
	for {
		...
		for i := 0; i < total; i++ {
			...
			cc := 1 + rng.Intn(4)
			oneCounter += cc
			s.AddCounter(&kCounter1, float64(cc), metricInfo1)
		}
		if spike {
		...
		if oneCounter < 3*totalRange*5 {
			...
			s.AddCounter(&kCounter1, float64(3*totalRange*5-oneCounter), metricInfo1)
		}
		...
		for i := 0; i < total; i++ {
			...
			for ci := 0; ci < cc; ci++ {
				...
				s.AddValueArrayCounterStringBytes(&kValue10, []float64{value}, 1, data_model.TagUnionBytes{S: sKey, I: 0}, metricInfo10)
				s.AddValueArrayCounterStringBytes(&kValue11, []float64{value}, 1, data_model.TagUnionBytes{S: sKey, I: 0}, metricInfo11)
			}
		}
		total = rng.Intn(totalRange)
		superTotal += total
		for i := 0; i < total; i++ {
			...
			for ci := 0; ci < cc; ci++ {
				...
				s.AddUniqueStringBytes(&kUnique4, data_model.TagUnionBytes{}, []int64{r}, 1, metricInfo4)
				s.AddUniqueStringBytes(&kUnique7, data_model.TagUnionBytes{}, []int64{r}, 1, metricInfo7)
				s.AddUniqueStringBytes(&kUnique8, data_model.TagUnionBytes{S: sKey, I: 0}, []int64{r}, 1, metricInfo8)
			}
		}
		...
		for i := 0; i < total; i++ {
			...
			if sid < len(stop) {
				s.AddCounterStringBytes(&kValue5, stop[sid], 1, metricInfo5)
				continue
			}
			cc := 1 + rng.Intn(100000)
			s.AddCounterStringBytes(&kValue5, []byte(fmt.Sprintf("A_%d", cc)), 1, metricInfo5)
		}
		total = rng.Intn(totalRange)
		superTotal += total
		for i := 0; i < total*100; i++ {
			...
			s.AddCounter(&kCounter6, 1, metricInfo6)
		}

		s.AddCounter(&data_model.Key{Metric: metricInfo9.MetricID}, float64(superTotal), metricInfo9)
		// msec := rng.Int() % 1000
