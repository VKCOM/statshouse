...
func (a *Aggregator) RowDataMarshalAppendPositions(b *aggregatorBucket, rnd *rand.Rand, res []byte, historic bool) []byte {
	...
	for k, v := range samplerStat.Items {
		// keep bytes
		key := a.aggKey(b.time, format.BuiltinMetricIDAggSamplingSizeBytes, [16]int32{0, key1, format.TagValueIDSamplingDecisionKeep, k[0], k[1], k[2]})
		mi := data_model.MultiItem{Tail: data_model.MultiValue{Value: v.SumSizeKeep}}
		insertItem(key, &mi, 1)
		// discard bytes
		key = a.aggKey(b.time, format.BuiltinMetricIDAggSamplingSizeBytes, [16]int32{0, key1, format.TagValueIDSamplingDecisionDiscard, k[0], k[1], k[2]})
	...
	// report budget used
	budgetKey := a.aggKey(b.time, format.BuiltinMetricIDAggSamplingBudget, [16]int32{0, key1})
	...
	for k, v := range samplerStat.Budget {
		key := a.aggKey(b.time, format.BuiltinMetricIDAggSamplingGroupBudget, [16]int32{0, key1, k[0], k[1]})
		item := data_model.MultiItem{}
		item.Tail.Value.AddValue(v)
		insertItem(key, &item, 1)
	}
	res = appendSimpleValueStat(res, a.aggKey(b.time, format.BuiltinMetricIDAggSamplingMetricCount, [16]int32{0, key1}),
		float64(len(samplerStat.Metrics)), 1, a.aggregatorHost, metricCache, usedTimestamps)
	res = appendSimpleValueStat(res, a.aggKey(b.time, format.BuiltinMetricIDAggInsertSize, [16]int32{0, 0, 0, 0, key1, format.TagValueIDSizeCounter}),
		float64(sizeCounters), 1, a.aggregatorHost, metricCache, usedTimestamps)
	res = appendSimpleValueStat(res, a.aggKey(b.time, format.BuiltinMetricIDAggInsertSize, [16]int32{0, 0, 0, 0, key1, format.TagValueIDSizeValue}),
		float64(sizeValues), 1, a.aggregatorHost, metricCache, usedTimestamps)
	res = appendSimpleValueStat(res, a.aggKey(b.time, format.BuiltinMetricIDAggInsertSize, [16]int32{0, 0, 0, 0, key1, format.TagValueIDSizePercentiles}),
		float64(sizePercentiles), 1, a.aggregatorHost, metricCache, usedTimestamps)
	res = appendSimpleValueStat(res, a.aggKey(b.time, format.BuiltinMetricIDAggInsertSize, [16]int32{0, 0, 0, 0, key1, format.TagValueIDSizeUnique}),
	...
	sizeBuiltin += len(res) - resPos
	resPos = len(res)
	res = appendSimpleValueStat(res, a.aggKey(b.time, format.BuiltinMetricIDAggInsertSize, [16]int32{0, 0, 0, 0, key1, format.TagValueIDSizeStringTop}),
	...
	res = appendSimpleValueStat(res, a.aggKey(b.time, format.BuiltinMetricIDAggInsertSize, [16]int32{0, 0, 0, 0, key1, format.TagValueIDSizeBuiltIn}),
