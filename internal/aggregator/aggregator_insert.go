...
func appendKeys(res []byte, k data_model.Key, metricCache *metricIndexCache, usedTimestamps map[uint32]struct{}, newFormat bool) []byte {
	...
	if prekeyIndex >= 0 {
		res = appendTag(res, uint64(k.Keys[prekeyIndex]))
		if prekeyOnly {
...
func appendMultiBadge(res []byte, k data_model.Key, v *data_model.MultiItem, metricCache *metricIndexCache, usedTimestamps map[uint32]struct{}, newFormat bool) []byte {
	if k.Metric >= 0 { // fastpath
		return res
	}
	for _, t := range v.Top {
		res = appendBadge(res, k, t.Value, metricCache, usedTimestamps, newFormat)
	}
	return appendBadge(res, k, v.Tail.Value, metricCache, usedTimestamps, newFormat)
}

func appendBadge(res []byte, k data_model.Key, v data_model.ItemValue, metricCache *metricIndexCache, usedTimestamps map[uint32]struct{}, newFormat bool) []byte {
	if k.Metric >= 0 { // fastpath
		return res
	}
	ts := (k.Timestamp / 5) * 5
	...
	case format.BuiltinMetricIDIngestionStatus:
		...
		case format.TagValueIDSrcIngestionStatusWarnDeprecatedKeyName,
			...
			return appendValueStat(res, data_model.Key{Timestamp: ts, Metric: format.BuiltinMetricIDBadges, Keys: [16]int32{0, format.TagValueIDBadgeIngestionWarnings, k.Keys[1]}}, "", v, metricCache, usedTimestamps, newFormat)
		}
		return appendValueStat(res, data_model.Key{Timestamp: ts, Metric: format.BuiltinMetricIDBadges, Keys: [16]int32{0, format.TagValueIDBadgeIngestionErrors, k.Keys[1]}}, "", v, metricCache, usedTimestamps, newFormat)
	case format.BuiltinMetricIDAgentSamplingFactor:
		return appendValueStat(res, data_model.Key{Timestamp: ts, Metric: format.BuiltinMetricIDBadges, Keys: [16]int32{0, format.TagValueIDBadgeAgentSamplingFactor, k.Keys[1]}}, "", v, metricCache, usedTimestamps, newFormat)
	case format.BuiltinMetricIDAggSamplingFactor:
		return appendValueStat(res, data_model.Key{Timestamp: ts, Metric: format.BuiltinMetricIDBadges, Keys: [16]int32{0, format.TagValueIDBadgeAggSamplingFactor, k.Keys[4]}}, "", v, metricCache, usedTimestamps, newFormat)
	case format.BuiltinMetricIDAggMappingCreated:
		...
		return appendValueStat(res, data_model.Key{Timestamp: ts, Metric: format.BuiltinMetricIDBadges, Keys: [16]int32{0, format.TagValueIDBadgeAggMappingErrors, k.Keys[4]}}, "", v, metricCache, usedTimestamps, newFormat)
	case format.BuiltinMetricIDAggBucketReceiveDelaySec:
		return appendValueStat(res, data_model.Key{Timestamp: ts, Metric: format.BuiltinMetricIDBadges, Keys: [16]int32{0, format.TagValueIDBadgeContributors, 0}}, "", v, metricCache, usedTimestamps, newFormat)
	}
	return res
}
...
func appendValueStat(res []byte, key data_model.Key, skey string, v data_model.ItemValue, cache *metricIndexCache, usedTimestamps map[uint32]struct{}, newFormat bool) []byte {
	...
	res = appendKeys(res, key, cache, usedTimestamps, newFormat)
...
func appendSimpleValueStat(res []byte, key data_model.Key, v float64, count float64, hostTag int32, metricCache *metricIndexCache, usedTimestamps map[uint32]struct{}, newFormat bool) []byte {
	return appendValueStat(res, key, "", data_model.SimpleItemValue(v, count, hostTag), metricCache, usedTimestamps, newFormat)
}
...
func (a *Aggregator) RowDataMarshalAppendPositions(buckets []*aggregatorBucket, rnd *rand.Rand, res []byte, newFormat bool) []byte {
	...
	insertItem := func(k data_model.Key, item *data_model.MultiItem, sf float64, bucketTs uint32) { // lambda is convenient here
		...
		if !item.Tail.Empty() { // only tail
			res = appendKeys(res, k, metricCache, usedTimestamps, newFormat)
	...
	for _, b := range buckets {
		...
		for si := 0; si < len(b.shards); si++ {
			for k, item := range b.shards[si].multiItems {
				...
				resPos := len(res)
				res = appendMultiBadge(res, k, item, metricCache, usedTimestamps, newFormat)
				is.builtin += len(res) - resPos
	...
	for _, s := range samplerStat.GetSampleFactors(nil) {
		...
		res = appendBadge(res, key, data_model.SimpleItemValue(sf, 1, a.aggregatorHost), metricCache, usedTimestamps, newFormat)
		res = appendSimpleValueStat(res, key, sf, 1, a.aggregatorHost, metricCache, usedTimestamps, newFormat)
	}
	...
	res = appendSimpleValueStat(res, a.aggKey(recentTime, format.BuiltinMetricIDAggSamplingMetricCount, [16]int32{0, historicTag}),
		float64(len(samplerStat.Metrics)), 1, a.aggregatorHost, metricCache, usedTimestamps, newFormat)
	...
	for t := range usedTimestamps {
		...
		res = appendSimpleValueStat(res, key, float64(insertTimeUnix)-float64(t), 1, a.aggregatorHost, metricCache, nil, newFormat)
