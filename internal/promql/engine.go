...
func NewEngine(opts EngineOpts) *Engine {
	if opts.LookbackDelta == 0 {
		...
		log.Printf("[debug] lookback delta is zero, setting to default value %v", defaultLookbackDelta)
	}
...
func (ev *evaluator) recover(ws *storage.Warnings, errp *error) {
	...
	switch err := e.(type) {
	case runtime.Error:
		log.Printf("[error] runtime panic in parser %v", err)
...
func putPointSlice(p []Point) {
	pointPool.Put(p[:0]) //lint:ignore SA6002 relax staticcheck verification.
}
