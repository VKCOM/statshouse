...
func (item *NetUdpPacketUnencHeader) ReadJSONGeneral(tctx *basictl.JSONReadContext, in *basictl.JsonLexer) error {
	...
	var propCryptoShaPresented bool
	var trueTypeCryptoShaValue bool
	var propCryptoRandomPresented bool
	var propEncryptedDataPresented bool
	var trueTypeEncryptedDataValue bool
	var propSupportMsgOffsetsPresented bool
	...
	if propCryptoShaPresented && !trueTypeCryptoShaValue && (item.Flags&(1<<4) != 0) {
		return ErrorInvalidJSON("netUdpPacket.unencHeader", "field 'crypto_sha' is explicitly set to false, but corresponding fieldmask item.Flags bit 4 is 1")
	}
	if propEncryptedDataPresented && !trueTypeEncryptedDataValue && (item.Flags&(1<<7) != 0) {
		return ErrorInvalidJSON("netUdpPacket.unencHeader", "field 'encrypted_data' is explicitly set to false, but corresponding fieldmask item.Flags bit 7 is 1")
	}
	if propSupportMsgOffsetsPresented && !trueTypeSupportMsgOffsetsValue && (item.Flags&(1<<8) != 0) {
		return ErrorInvalidJSON("netUdpPacket.unencHeader", "field 'support_msg_offsets' is explicitly set to false, but corresponding fieldmask item.Flags bit 8 is 1")
	}
	return nil
}
