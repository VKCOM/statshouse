...
func (item *BarsicReindex) ReadJSONGeneral(tctx *basictl.JSONReadContext, in *basictl.JsonLexer) error {
	...
	if trueTypeFastPresented && !trueTypeFastValue && (item.FieldsMask&(1<<0) != 0) {
		return internal.ErrorInvalidJSON("barsic.reindex", "fieldmask bit item.FieldsMask.0 is indefinite because of the contradictions in values")
	}
	// tries to set bit to zero if it is 1
	if trueTypeDiffPresented && !trueTypeDiffValue && (item.FieldsMask&(1<<1) != 0) {
		return internal.ErrorInvalidJSON("barsic.reindex", "fieldmask bit item.FieldsMask.1 is indefinite because of the contradictions in values")
	}
	return nil
}
