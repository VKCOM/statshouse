...
func HammerWeighted(sem *semaphore.Weighted, n int64, loops int) {
	for i := 0; i < loops; i++ {
		sem.Acquire(context.Background(), n) //nolint:errcheck
...
func TestWeightedTryAcquire(t *testing.T) {
	...
	tries := []bool{}
	sem.Acquire(ctx, 1) //nolint:errcheck
...
func TestWeightedDoesntBlockIfTooBig(t *testing.T) {
	...
	sem := semaphore.NewWeighted(n)
	{
		...
		defer cancel()
		go sem.Acquire(ctx, n+1) //nolint:errcheck
	}
...
func TestLargeAcquireDoesntStarve(t *testing.T) {
	...
	sem.Acquire(ctx, n) //nolint:errcheck
	running = false
	sem.Release(n)
	wg.Wait()
}
...
func TestAllocCancelDoesntStarve(t *testing.T) {
	...
	sem.Acquire(context.Background(), 1) //nolint:errcheck

	// In the background, Acquire(_, 10).
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	go func() {
		sem.Acquire(ctx, 10) //nolint:errcheck
	}()
