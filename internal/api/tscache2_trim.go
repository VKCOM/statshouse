...
func (tr *cache2Trim) reduceMemoryUsage() int {
	...
	for _, m := range c.shards {
		for _, m := range m {
			...
			for v != nil {
				h.push(cache2TrimBucket{v.runtimeInfo(), m, v})
	...
	tr.h = h.clear()
	return c.runtimeInfo().sizeInBytes
}
...
func (h cache2TrimBucketHeap) less(i, j int) bool {
	l, r := &h[i].r, &h[j].r
	if v := cmp.Compare(l.play, r.play); v != 0 {
	...
	return cmp.Compare(l.sizeInBytes, r.sizeInBytes) > 0
}
