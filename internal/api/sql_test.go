...
func TestTagValuesQueryV2(t *testing.T) {
	...
	assert.Equal(t, "SELECT key2 AS _value, toFloat64(sum(count)) AS _count FROM statshouse_value_1m_dist WHERE time >= ? AND time < ? AND metric = 1000 AND key1 IN (1, 2) AND key0 NOT IN (3) GROUP BY key2 HAVING _count > 0 ORDER BY _count DESC,_value LIMIT 6 SETTINGS optimize_aggregation_in_order=1", query.body)
}
...
func TestTagValuesQueryV2_stringTop(t *testing.T) {
	...
	assert.Equal(t, "SELECT skey AS _string_value, toFloat64(sum(count)) AS _count FROM statshouse_value_1m_dist WHERE time >= ? AND time < ? AND metric = 1000 AND key1 IN (1, 2) AND key0 NOT IN (3) GROUP BY skey HAVING _count > 0 ORDER BY _count DESC,_string_value LIMIT 6 SETTINGS optimize_aggregation_in_order=1", query.body)
}
...
func TestLoadPointsQueryV2_maxHost(t *testing.T) {
	...
	// checks
	assert.NoError(t, err)
	assert.Equal(t, 6, query.vals)
	assert.True(t, query.minMaxHost)
	assert.Equal(t, "2", query.version)
	assert.Empty(t, query.tags)
	assert.Equal(t, "SELECT toInt64(toStartOfInterval(time + 10800, INTERVAL 60 second)) - 10800 AS _time, toInt64(60) AS _stepSec, toFloat64(sum(count)) AS _count, toFloat64(min(min)) AS _val0, toFloat64(max(max)) AS _val1, toFloat64(sum(sum))/toFloat64(sum(count)) AS _val2, toFloat64(sum(sum)) AS _val3, if(sum(count) < 2, 0, sqrt(greatest(   (sum(sumsquare) - pow(sum(sum), 2) / sum(count)) / (sum(count) - 1)   , 0))) AS _val4, toFloat64(sum(1)) AS _val5, argMinMerge(min_host) as _minHost, argMaxMerge(max_host) as _maxHost FROM statshouse_value_1m_dist WHERE time >= ? AND time < ? AND metric = 1000 AND key1 IN (1, 2) AND key0 NOT IN (3) GROUP BY _time LIMIT 10000000 SETTINGS optimize_aggregation_in_order=1", query.body)
}
...
func TestLoadPointsQueryV3(t *testing.T) {
	...
	assert.Empty(t, query.tags)
	assert.Equal(t, `SELECT toInt64(toStartOfInterval(time + 10800, INTERVAL 60 second)) - 10800 AS _time, toInt64(60) AS _stepSec, toFloat64(sum(count)) AS _count, toFloat64(sum(1)) AS _val0, toFloat64(max(max)) AS _val1 FROM statshouse_v3_1m_dist WHERE index_type = 0 AND time >= ? AND time < ? AND metric = 1000 AND (tag1 IN (1, 2) OR stag1 IN ('one', 'two')) AND (tag0 NOT IN (0) AND stag0 NOT IN ('staging')) GROUP BY _time LIMIT 10000000 SETTINGS optimize_aggregation_in_order=1`, query.body)
}
