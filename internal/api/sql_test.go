...
func getLodForV5(t *testing.T, start int64, end int64, time_now int64, utc_offset int64) data_model.LOD {
	lods, err := data_model.GetLODs(data_model.GetTimescaleArgs{
		Version:       Version3,
		Version3Start: 1,
		Version5Start: 2,
...
func TestLoadPointsQueryV5_1h(t *testing.T) {
	// prepare
	pq := queryBuilder{
		metric: metric,
		user:   "test-user",
	...
	pq.filterIn.Append(1, data_model.NewTagValue("one", 1), data_model.NewTagValue("two", 2))
	pq.filterNotIn.AppendValue(0, "staging")
	lod := getLodForV5(t, 100_000, 2_000_000, 2_000_000, 3)
...
func TestLoadPointsQueryV5_1m(t *testing.T) {
	// prepare
	pq := queryBuilder{
		metric: metric,
		user:   "test-user",
	...
	pq.filterIn.Append(1, data_model.NewTagValue("one", 1), data_model.NewTagValue("two", 2))
	pq.filterNotIn.AppendValue(0, "staging")
	lod := getLodForV5(t, 10_000, 20_000, 20_000, 3)
...
func TestLoadPointsQueryV5_1s(t *testing.T) {
	// prepare
	pq := queryBuilder{
		metric: metric,
		user:   "test-user",
	...
	pq.filterIn.Append(1, data_model.NewTagValue("one", 1), data_model.NewTagValue("two", 2))
	pq.filterNotIn.AppendValue(0, "staging")
	lod := getLodForV5(t, 10001, 10030, 10030, 0)
