...
func newCache2(h *Handler) *cache2 {
	...
	for version, v := range data_model.LODTables {
		...
		for stepSec := range v {
			...
			res.info.sizeInBytes += sizeofCache2Shard
		}
	}
	go res.trim()
	return res
...
func (c *cache2) updateRuntimeInfoUnlocked(info cache2UpdateInfo) {
	...
	// memory usage
	c.info.sizeInBytes += info.sizeInBytesDelta
...
func (c *cache2) memoryUsageWithinLimit() (int, bool) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *cache2) memoryUsageWithinLimitUnlocked() (int, bool) {
	return c.info.sizeInBytes, c.limits.maxSizeInBytes <= 0 || c.info.sizeInBytes <= c.limits.maxSizeInBytes
}
...
func (s *cache2Shard) getOrCreateBucket(q *queryBuilder, info *cache2UpdateInfo) *cache2Bucket {
	...
	s.bucketM[k] = v
	s.bucketL.add(v)
	info.sizeInBytesDelta += v.sizeInBytesUnlocked()
	return v
}
...
func (s *cache2Shard) removeBucket(v *cache2Bucket, info *cache2UpdateInfo) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.removeBucketUnlocked(v, info)
}

func (s *cache2Shard) removeBucketUnlocked(v *cache2Bucket, info *cache2UpdateInfo) {
	s.bucketM[v.key] = nil
	delete(s.bucketM, v.key)
	if s.trimIter == v {
		s.trimIter = s.bucketL.next(v)
	}
	if s.invalidateIter == v {
		s.invalidateIter = s.bucketL.next(v)
	}
	s.bucketL.remove(v)
	v.clearAndDetach(info)
}
...
func (b *cache2Bucket) sizeInBytes() int {
	b.mu.Lock()
	defer b.mu.Unlock()
	return sizeofCache2Bucket +
		2*len(b.key) + // count both map key and value key
		sizeofCache2Chunks(b.chunks)
}

func (b *cache2Bucket) sizeInBytesUnlocked() int {
	return sizeofCache2Bucket +
		2*len(b.key) + // count both map key and value key
		sizeofCache2Chunks(b.chunks)
}
