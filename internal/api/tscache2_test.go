	...
	rapid.Check(t, func(t *rapid.T) {
		...
		for i := 0; i < 10; i++ {
			h = h.push(cache2TrimBucket{r: cache2BucketRuntimeInfo{
				sizeInBytes: rapid.Int().Draw(t, "sizeInBytes"),
			}})
		}
		sizeInBytes := h.min().r.sizeInBytes
		for h.len() > 1 {
			h = h.pop()
			require.GreaterOrEqual(t, sizeInBytes, h.min().r.sizeInBytes)
			sizeInBytes = h.min().r.sizeInBytes
		}
	})
}
...
func TestCache2TrimBucketHeapMaxPlay(t *testing.T) {
	rapid.Check(t, func(t *rapid.T) {
		...
		for i := 0; i < 1000; i++ {
			h = h.push(cache2TrimBucket{r: cache2BucketRuntimeInfo{
				play: rapid.Int().Draw(t, "play"),
			}})
		}
		play := h.min().r.play
		for h.len() > 1 {
			h = h.pop()
			require.GreaterOrEqual(t, play, h.min().r.play)
			play = h.min().r.play
		}
	})
}
