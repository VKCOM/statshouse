...
func (s *HTTP) Serve(h Handler, ln net.Listener) error {
	...
	handler.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		http.Error(w, "To send events via HTTP, use /api/statshousev1 endpoint and send event in http body. All event formats (JSON, TL, Protobuf, etc.) supported via UDP/TCP also work via HTTP.", http.StatusNotFound)
	})
