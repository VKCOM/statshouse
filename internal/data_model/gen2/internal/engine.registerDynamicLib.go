...
func (item *EngineRegisterDynamicLib) writeResultJSON(short bool, w []byte, ret BoolStat) (_ []byte, err error) {
...
func (item *EngineRegisterDynamicLib) ReadResultWriteResultJSONShort(r []byte, w []byte) (_ []byte, _ []byte, err error) {
	var ret BoolStat
	if r, err = item.ReadResult(r, &ret); err != nil {
		return r, w, err
	}
...
func (item *EngineRegisterDynamicLib) WriteJSONOpt(short bool, w []byte) (_ []byte, err error) {
	w = append(w, '{')
	if len(item.LibId) != 0 {
