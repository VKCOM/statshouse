...
func (item *EngineHttpQueryResponse) ClearAdditionalHeaders() {
	BuiltinDictStringStringReset(item.AdditionalHeaders)
	item.FieldsMask &^= 1 << 3
}
...
func (item *EngineHttpQueryResponse) FillRandom(rg *basictl.RandGenerator) {
	...
	if item.FieldsMask&(1<<0) != 0 {
		item.ReturnCode = basictl.RandomInt(rg)
	} else {
		item.ReturnCode = 0
	}
	...
	if item.FieldsMask&(1<<2) != 0 {
		item.ContentType = basictl.RandomString(rg)
	} else {
		item.ContentType = ""
	}
	if item.FieldsMask&(1<<3) != 0 {
		BuiltinDictStringStringFillRandom(rg, &item.AdditionalHeaders)
	} else {
		BuiltinDictStringStringReset(item.AdditionalHeaders)
	}
}

func (item *EngineHttpQueryResponse) ReadTL1(w []byte) (_ []byte, err error) {
	...
	if item.FieldsMask&(1<<3) != 0 {
		if w, err = BuiltinDictStringStringReadTL1(w, &item.AdditionalHeaders); err != nil {
			return w, err
		}
	} else {
		BuiltinDictStringStringReset(item.AdditionalHeaders)
...
func (item *EngineHttpQueryResponse) WriteTL1General(w []byte) (_ []byte, err error) {
	return item.WriteTL1(w), nil
}

func (item *EngineHttpQueryResponse) WriteTL1(w []byte) []byte {
	...
	if item.FieldsMask&(1<<3) != 0 {
		w = BuiltinDictStringStringWriteTL1(w, item.AdditionalHeaders)
	}
	return w
}

func (item *EngineHttpQueryResponse) ReadTL1Boxed(w []byte) (_ []byte, err error) {
...
func (item *EngineHttpQueryResponse) WriteTL1BoxedGeneral(w []byte) (_ []byte, err error) {
	return item.WriteTL1Boxed(w), nil
}

func (item *EngineHttpQueryResponse) WriteTL1Boxed(w []byte) []byte {
