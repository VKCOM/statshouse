...
func (item StatshouseApiQuery) RepairMasksValue() StatshouseApiQuery {
	item.RepairMasks()
	return item
}
func (item *StatshouseApiQuery) RepairMasks() {
	item.tl2mask0 = 0
	if item.FieldsMask&(1<<0) != 0 {
		item.tl2mask0 |= 1
	}
...
func (item *StatshouseApiQuery) ReadTL1(w []byte) (_ []byte, err error) {
	item.tl2mask0 = 0
	...
	if w, err = item.Function.ReadTL1Boxed(w); err != nil {
		return w, err
	}
	if w, err = BuiltinVectorStringReadTL1(w, &item.GroupBy); err != nil {
		return w, err
	}
	if w, err = BuiltinVectorStatshouseApiFilterReadTL1(w, &item.Filter); err != nil {
		return w, err
	}
	if w, err = BuiltinVectorLongReadTL1(w, &item.TimeShift); err != nil {
		return w, err
	}
	if item.FieldsMask&(1<<0) != 0 {
		item.tl2mask0 |= 1
	...
	if item.FieldsMask&(1<<1) != 0 {
		item.tl2mask0 |= 2
		if w, err = BuiltinVectorStatshouseApiFunctionReadTL1(w, &item.What); err != nil {
			return w, err
		}
	} else {
		item.What = item.What[:0]
...
func (item *StatshouseApiQuery) WriteTL1General(w []byte) (_ []byte, err error) {
	return item.WriteTL1(w), nil
}

func (item *StatshouseApiQuery) WriteTL1(w []byte) []byte {
	...
	w = item.Function.WriteTL1Boxed(w)
	w = BuiltinVectorStringWriteTL1(w, item.GroupBy)
	w = BuiltinVectorStatshouseApiFilterWriteTL1(w, item.Filter)
	w = BuiltinVectorLongWriteTL1(w, item.TimeShift)
	if item.FieldsMask&(1<<0) != 0 {
		w = basictl.StringWrite(w, item.Promql)
	}
	if item.FieldsMask&(1<<1) != 0 {
		w = BuiltinVectorStatshouseApiFunctionWriteTL1(w, item.What)
	}
	if item.FieldsMask&(1<<3) != 0 {
...
func (item *StatshouseApiQuery) ReadTL1Boxed(w []byte) (_ []byte, err error) {
...
func (item *StatshouseApiQuery) WriteTL1BoxedGeneral(w []byte) (_ []byte, err error) {
	return item.WriteTL1Boxed(w), nil
}

func (item *StatshouseApiQuery) WriteTL1Boxed(w []byte) []byte {
...
func (item *StatshouseApiQuery) ReadJSONGeneral(tctx *basictl.JSONReadContext, in *basictl.JsonLexer) error {
	...
	var propExcessPointsFlagPresented bool
	var trueTypeExcessPointsFlagValue bool
	var propWidthAggPresented bool
	var propNameFlagPresented bool
	var trueTypeNameFlagValue bool
	var propColorFlagPresented bool
	var trueTypeColorFlagValue bool
	var propTotalFlagPresented bool
	var trueTypeTotalFlagValue bool
	var propMaxHostFlagPresented bool
	...
	if !propPromqlPresented && item.FieldsMask&(1<<0) != 0 {
		item.tl2mask0 |= 1
	}
	if !propWhatPresented && item.FieldsMask&(1<<1) != 0 {
		item.tl2mask0 |= 2
	}
	if !propExcessPointsFlagPresented && item.FieldsMask&(1<<2) != 0 {
		item.tl2mask0 |= 4
	}
	if !propWidthAggPresented && item.FieldsMask&(1<<3) != 0 {
		item.tl2mask0 |= 8
	}
	if !propNameFlagPresented && item.FieldsMask&(1<<4) != 0 {
		item.tl2mask0 |= 16
	}
	if !propColorFlagPresented && item.FieldsMask&(1<<5) != 0 {
		item.tl2mask0 |= 32
	}
	if !propTotalFlagPresented && item.FieldsMask&(1<<6) != 0 {
		item.tl2mask0 |= 64
	}
	if !propMaxHostFlagPresented && item.FieldsMask&(1<<7) != 0 {
		item.tl2mask0 |= 128
	}
	if propPromqlPresented {
		item.FieldsMask |= 1 << 0
