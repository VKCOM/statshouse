...
import (
	...
	sqlitev2 "github.com/vkcom/statshouse/internal/sqlite"
...
func main() {
	...
	db, err := sqlitev2.OpenEngine(sqlitev2.Options{
		...
		DurabilityMode:    sqlitev2.NoWaitCommit,
	}, bl, func(conn sqlitev2.Conn, offset int64, payload []byte) (int, error) {
		n, err := apply(conn, payload)
		if err != nil {
			log.Printf("ERROR TO APPLY, %d, %d", offset, offset+int64(n))
		}
		return n, err
	}, nil)
	if err != nil {
