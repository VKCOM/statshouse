...
Чтобы изучить взаимосвязи между метриками или событиями, используйте функцию [Event overlay](#11--наложение-событий).
...
В этом меню могут отображаться статистики, не релевантные для вашей метрики. Если их выбрать, на графике 
появятся `0`-значения. Чтобы отключить отображение нерелевантных статистик,
[укажите тип метрики в разделе редактирования метрики](edit-metrics.md#aggregation).
...
Включить запись перцентилей можно в [разделе редактирования метрики](edit-metrics.md#percentiles).
...
По умолчанию параметр _Live mode_ не содержится в URL-адресе метрики.

:::tip
Чтобы отправить ссылку на график с включенным параметром _Live mode_, добавьте строку `live=1` в URL метрики.
:::
...
:::tip
Если у вашей метрики меньше 16 тегов, ненужные теги можно [скрыть](edit-metrics.md#tags).
:::
...
Предствьте: обычно данные варьируются в диапазоне от 1 до 100. Иногда появляются пики. StatsHouse обычно подстраивает
масштаб оси Y под размер пика. Это бывает не очень удобно, когда вас интересуют не пики, а остальные данные.
