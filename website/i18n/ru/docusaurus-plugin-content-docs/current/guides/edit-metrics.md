...
Узнайте, [какие параметры метрики можно редактировать](#how-to-edit-a-metric):
...
вашей метрики была минимальной, или уменьшите её [разрешение](edit-metrics.md#разрешение).
...
(алиасы). Прежде чем упоминать алиас, настройте его:
...
Чтобы не забыть, что означают "сырые" значения тегов, укажите [формат](#specifying-formats-for-raw-tag-values), 
который будет отображаться в интерфейсе, и добавьте [комментарии](#value-comments).
...
не [настраивали их в интерфейсе](#describe-tags). Функциональность драфт-тегов помогает быстрее 
настраивать теги.
:::
...
Эта метрика удаляется из списка метрик.
