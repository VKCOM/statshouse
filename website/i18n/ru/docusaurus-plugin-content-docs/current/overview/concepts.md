...
ниже: для каждой полученной комбинации значений тегов мы получим ряд с соответствующим счётчиком:
...
статистик. Из-за этого [счётчики могут принимать нецелые значения](../guides/view-graph.md#3--descriptive-statistics),
...
Количество занятых байтов зависит от [типа метрики](../guides/design-metric.md#metric-types)
...
[веса](../guides/edit-metrics.md#admin-settings) для конкретных метрик.
...
При семплировании StatsHouse первую половину бюджета (два ряда в данном примере) расходует на ряды с 
максимальным значением счётчика без семплирования. Вторая половина бюджета потратится на случайные из оставшихся рядов 
(они будут семплироваться пропорционально сильнее).
...
Лучше, когда [семплированием](#sampling) данных занимается StatsHouse. Однако вам может быть нужно 
...
[агрегаты](#aggregation) доступны в данные момент: посекундные данные можно получить в течение двух последних дней, 
...
[семплирование](#sampling), [уменьшите разрешение метрики](../guides/edit-metrics.md#resolution).

Например, вы можете выбрать такое разрешение, чтобы [агент](components.md#agent)
