...
* Для Service A: 9 из 10 рядов попадают в базу данных (не семплируются).
* Для Service B: только один ряд не будет выброшен из-за семплирования (с вероятностью 10%).
