...
Узнайте больше о том, [как устроены метрики в StatsHouse](guides/design-metric.md#what-are-metrics-in-statshouse) и 
...
[как искать метрики и как не испортить чужую метрику](guides/view-graph.md#1--metric-name). 

### Описательные статистики

Это статистические функции, которые количественно описывают или обобщают данные.
Можно выбрать одну или несколько статистик. Вот самые распространённые из них:
...
[Зависит от типа метрики.](guides/design-metric.md#metric-types)
...
Данные за какой-то период времени можно сравнить с данными за аналогичный период в прошлом: на день, неделю или год 
раньше.
...
Интервал [агрегации](overview/concepts.md#aggregation) — это что-то вроде разрешения для вашей метрики. Чем больше интервал 
агрегации, тем более сглаженным выглядит график:

<img src={AggregationInterval} width="600"/>

:::tip
Если выбрать значение _Auto_, для отображения данных на графике будет использоваться [_минимальный интервал 
агрегации из доступных_](overview/concepts.md#minimal-available-aggregation-interval).
...
Доступный интервал агрегации также связан с [разрешением](overview/concepts.md#resolution) метрики.
:::

Узнайте больше об [агрегации](overview/concepts.md#aggregation) в StatsHouse и о том, как менять
...
или увеличивать [семплирование](guides/view-graph.md#sampling) из-за высокой [кардинальности](overview/concepts.md#cardinality).
Если вам всё равно нужны такие теги, почитайте
[о них подробнее](guides/design-metric.md#how-many-tag-values).
:::

Узнайте больше о том, [как фильтровать данные с помощью тегов](guides/view-graph.md#7--tags),
[как их настраивать](guides/design-metric.md#tags) 
и [редактировать](guides/edit-metrics.md#tags), а также о том, что такое 
[кардинальность](overview/concepts.md#cardinality).
