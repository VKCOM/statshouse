...
export type VariableSourceProps = {
  ...
  indexValue?: number;
  onChange?: (indexValue: number, value?: VariableParamsSource) => void;
};
export function VariableSource({ value, indexValue = 0, onChange }: VariableSourceProps) {
  const uid = useId();
  ...
  const onSetUpdateTag = useCallback(
    (tagKey: TagKey | undefined, updated: boolean) => {
      if (localMetric && tagKey) {
        ...
        setUpdatedSource({ metric: localMetric, tag: tagKey, filterNotIn, filterIn }, updated);
      }
    },
    [localFilterIn, localFilterNotIn, localMetric]
  );
  ...
  useEffect(() => {
    if (!localMetric) {
      ...
      useStore.getState().loadMetricsMeta(localMetric);
  ...
  }, [indexValue, localFilterIn, localFilterNotIn, localMetric, localTag, onChange]);
