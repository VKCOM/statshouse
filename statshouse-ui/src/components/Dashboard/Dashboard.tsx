...
export const Dashboard: React.FC<DashboardProps> = ({ embed = false, yAxisSize = 54 }) => {
  ...
  return (
    <>
      <div className={cn(select !== null ? css.cursorDrag : css.cursorDefault)} ref={zone}>
        ...
        {itemsGroup.map((group, indexGroup) => (
          <div key={indexGroup} className="pb-5" data-group={indexGroup}>
            <h6
              hidden={
                itemsGroup.length <= 1 && !dashboardLayoutEdit && !params.dashboard?.groupInfo?.[indexGroup]?.name
              }
