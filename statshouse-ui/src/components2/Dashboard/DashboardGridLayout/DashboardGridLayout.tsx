...
import { LAYOUT_WIDGET_SIZE } from '@/api/enum';
...
export function DashboardGroupLayout({
  ...
  // const [dragItem, setDragItem] = useState<Layout>();
  // const [layouts, setLayout] = useState<Layouts>({ [breakpoint]: [] });
  // useEffect(() => {
  // console.log(items);
  // }, [items]);
  const onBreakpointChange = useCallback((newBreakpoint: string, _newCols: number) => {
  ...
  const layouts = useMemo(
    ...
    // return { [BREAKPOINT]: items, [BREAKPOINT_MOBILE]: items };
    [items]
  );
