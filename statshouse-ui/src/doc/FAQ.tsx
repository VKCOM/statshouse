...
const FAQ = React.memo(function FAQ_(props: { yAxisSize: number }) {
  ...
  return (
    ...
    statlogsUniqueCount(self::STATS_USERS_KEY, $user_id, $block, $action, test_getFeedRightAppsBlockGroup($user_id));`}</code>
        ...
        <p>
          Установлено ограничение в 300 ключей, который восполняется по 48,
          ключей в день. То есть если вы долго не создавали ключей, можете сразу создать 300, но после этого только 48 в
          день. Если не создавать ключи долго, бюджет восстановится.
        </p>
        <p>
          После того, как строковые ключи отображены в целочисленные, при вставке в базу получится какое-то количество
          строк, по одной на каждую использованную комбинацию ключей. На каждую метрику выделяется бюджет, зависящий от других метрик
          и других условий. Если попытаться вставлять больше данных, чем бюджет - произойдет сэмплирование, то есть часть рядов будет выкинуты,
          а оставшиеся умножены на выбранный фактор
        </p>
        <h5 id="sampling">
        ...
        <p>
          В минуту для каждой метрики система выделяет динамически некоторый бюджет. Этот бюджет можно потратить по-разному,
        ...
        <p>
          В StatsHouse PHP окружение записывается как значение отдельного ключа с именем <mark>0</mark>, поэтому оно подчиняется
        ...
        <p>
          ClickHouse это <abbr title="Log Structured Merge tree">LSM</abbr> база, данные хранятся в виде партов,
          для секундной таблицы с правилом <mark>partition by 6 hours</mark>, для остальных с правилом <mark>partition by 1 day</mark>.
          Если вставить старые данные, то придётся создать более 1 парта, например если вставить данные за прошлую неделю, то будет
          создано несколько десятков партов, вместо трёх, что создаст огромную нагрузку на ZooKeeper и сам ClickHouse.
        </p>
        <p>
          Вставка старых данных это непропорциональный объём дополнительной работы для базы, в нашем эксперименте тестовая запись
          всего 100 килобайтов данных за старые даты привела к отказу кластера статлогов на 7 часов.
        </p>
        <p>
          Вы можете явно указывать timestamp, однако если он в будущем, то он заменяется текущим временем, а
          если в прошлом более, чем на 26 часов, то заменяется на текущее время минус 26 часов. Таким образом крон,
          запускающийся раз в сутки, может вставить статистику за прошлые сутки с правильным временем, если сам крон работает менее двух часов.
        </p>
        <h5 id="raw">
